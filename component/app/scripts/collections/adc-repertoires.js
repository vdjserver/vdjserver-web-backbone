    ...
    countByDiagnosis(field) {
        ...
        for (var i = 0; i < this.length; ++i) {
            ...
            for (var j = 0; j < subjects.length; ++j) {
                var subject = subjects.at(j);
                var value = subject.get('value');
                var diagnoses = value.diagnosis;
                for (var k = 0; k < diagnoses.length; ++k) {
                    var diagnosis = diagnoses.at(k);
                    if (paths[1]=='disease_diagnosis') field = diagnosis[paths[1]].label;
                    else var field = diagnosis[paths[1]];
                    if (field == null) field = "null";
                    var entry = counts[field];
                    if (entry == null) counts[field] = 1;
                    else counts[field] += 1;
